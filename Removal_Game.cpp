...
int fun(int i, int j, vector<int> &a, vector<vi> &dp)
{
    if (i == j)
        return a[i];
    ...
    int op1 = a[i] + query(i + 1, j) - fun(i + 1, j, a, dp);
    int op2 = a[j] + query(i, j - 1) - fun(i, j - 1, a, dp);
...
int32_t main()
{
    ...
    vector<int> a(n + 1);
    vector<vi> dp(n + 1, vector<int>(n + 1, 1e15));
